...
export const useHistory = () => {
     ...
     */
    const initDatabase = useCallback((): Promise<void> => {
        return new Promise((resolve, reject) => {
            if (db) {
                resolve(); // If the database is already initialized, resolve immediately
                return;
            }
    ...
    useEffect(() => {
        initDatabase()
            .then(() => fetchHistory())
            .catch(error => {
                console.error('Failed to initialize and fetch history', error);
            });
    }, [initDatabase, fetchHistory]);
